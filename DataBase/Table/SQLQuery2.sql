...
Alter PROC SP_AUDITS
    ...
    @AUDIT_TO DATETIME='31/12/9999',
    @PROCESSED_FROM DATETIME= '1/1/1753',
     @PROCESSED_TO DATETIME='31/12/9999'
	AS
BEGIN
	SELECT 
		BATCH_NO,
    ...
    AND NO_OF_RECORDS=ISNULL(@NO_OF_RECORDS,'')
    AND AUDITOR_COMMENTS=ISNULL(@AUDITOR_COMMENTS,'')
    AND IS_DEFECT=ISNULL(@IS_DEFECT,'')
    AND [STATUS]=ISNULL(@STATUS,'')
